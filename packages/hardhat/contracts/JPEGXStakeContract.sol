...
contract JPEGXStakeContract is ERC721Holder, Auction {
    ...
    event Staked(address indexed owner, uint256 id, uint256 time, uint256 strikePrice);
    event Executeddeal(address indexed caller, uint256 id, uint256 time);
    event Dealstarted(address indexed owner, uint256 value, uint256 id, uint256 time);
    ...
    function stakerIn(uint256 _strikePrice, uint256 _tokenId) public nonReentrant {
        ...
        require(token.ownerOf(_tokenId) == address(this), "This token isn't deposited");
        require(tokenId_owner[_tokenId] == msg.sender, "You didn't deposit this NFT");
    ...
    function startDeal(uint256 _tokenId) public payable nonReentrant {
        ...
        require(msg.value == optionPrice, "msg.value doesn't match the option price");
    ...
    function executeDeal(uint256 _tokenId) public {
        ...
        require(block.timestamp > endingTimestamp, "Option period isn't expired yet");
        ...
        if (marketPrice > strikePrice) {
            ...
            tokenId_coverPositionExpiry[_tokenId] = block.timestamp + coverPositionExpiry;
    ...
    function coverPosition(uint256 _tokenId) public payable nonReentrant {
        require(tokenId_liquidable[_tokenId] == true, "NFT is not in liquidation");
        require(
